...
unknown, it must not clear attachments; it may only dismiss the modal.
...
Use an existing logical alias only as a human-readable label. Use a new ChatGPT
conversation for clean critique, independent tasks, or health checks. Aliases
are repo-owned and task-scoped by default. `main` means this repo plus this
Codex task's main room, not a globally shared `main`.
...
Use this task's active repo-owned main room.

```bash
chatgpt-pro call --alias polymarket-lp --task-id lp-release-review --project-url https://chatgpt.com/g/...
```

Open a new conversation inside that ChatGPT Project and bind `polymarket-lp` to
this task's effective room alias. This is the daily advisor/reviewer default
for Project work: same Project, separate conversation per Codex task.

```bash
chatgpt-pro call --alias polymarket-lp --task-id lp-release-review --project-url https://chatgpt.com/g/... --reuse-room
```

Reuse this same task's previously bound Project conversation deliberately.

```bash
chatgpt-pro call --alias polymarket-lp --shared-room --reuse-room
```

Deliberately use the exact shared `polymarket-lp` room across tasks/agents.
This is not the default because it can mix unrelated tasks.
